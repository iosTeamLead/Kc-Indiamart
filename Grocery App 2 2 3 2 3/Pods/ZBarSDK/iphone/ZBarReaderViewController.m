...
readerView, scanCrop,backButton;
...
- (void) initControls
{
    ...
    controls.backgroundColor = [UIColor colorWithRed:129/255.0f green:45/255.0f blue:33/255.0f alpha:1];
    
    UILabel *autoAddLabel = [UILabel new];
    
    //    autoAddLabel.frame = CGRectMake(60, 20, 100, 20);
    autoAddLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    autoAddLabel.textColor = [UIColor whiteColor];
    //    autoAddLabel.backgroundColor = [UIColor redColor];
    
    //    CGFloat width =  [label.text sizeWithFont:[UIFont fontWithName:@"ChaparralPro-Bold" size:40 ]].width;
    
    //use this for system font
    CGFloat width =  [@"Auto Add to List" sizeWithFont:[UIFont boldSystemFontOfSize:15.0]].width;
    autoAddLabel.adjustsFontSizeToFitWidth =YES;
    autoAddLabel.frame = CGRectMake(60, 20, width,20);
    autoAddLabel.text = @"Auto Add to List";
    [controls addSubview: autoAddLabel];
    
    UISwitch *onOffSwitch = [UISwitch new];
    onOffSwitch.frame= CGRectMake(autoAddLabel.frame.origin.x+autoAddLabel.frame.size.width+5, 20, 0, 0);
    onOffSwitch.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    [onOffSwitch addTarget:self action:@selector(onOffSwicthAxn:) forControlEvents:UIControlEventValueChanged];
    [onOffSwitch bringSubviewToFront:self.view];
    [controls addSubview: onOffSwitch];
    
    bool previuosValue=[[NSUserDefaults standardUserDefaults]boolForKey:@"isSwitchOn"];
    ...
    //    [toolbar release];
    //Bottom Control Harry
    [view addSubview: controls];
}
-(void)onOffSwicthAxn:(UISwitch*)sender{
    
    if ([sender isOn]) {
        NSLog(@"on");
        [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"isSwitchOn"];
    ...
    {
        NSLog(@"off");
        //        kSetBoolValueForKey(@"isSwitchOn", NO);
        
        [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"isSwitchOn"];
        [[NSUserDefaults standardUserDefaults] synchronize];
...
- (void) loadView
{
    ...
    self.view = [[UIView alloc]
                 initWithFrame: CGRectMake(0, 20, screenWidth, screenHeight)];
}
...
- (void) viewDidLoad
{
    ...
    UIView *backgroungBlurrView = [[UIView alloc] initWithFrame:view.frame];
    backgroungBlurrView.backgroundColor = [UIColor clearColor];
    
    //    [backgroungBlurrView addSubview:cameraOverlayView];
    [view addSubview:backgroungBlurrView];
    
    
    
    UIView *topView =[[UIView alloc] initWithFrame:CGRectMake(0,20, screenWidth, 45)];
    //    topView.autoresizingMask =
    //    UIViewAutoresizingFlexibleWidth |
    //    UIViewAutoresizingFlexibleHeight;
    topView.backgroundColor =  [UIColor colorWithRed:129/255.0f green:45/255.0f blue:33/255.0f alpha:1];
    
    backButton = [[UIButton alloc] initWithFrame:CGRectMake( 0, 0, 80, 45)];
    ...
    CGSize expectedLabelSize = [@"Keep the product upright and in good Lighting for better scanning" sizeWithFont:[UIFont systemFontOfSize:20.0] constrainedToSize:maximumLabelSize lineBreakMode:NSLineBreakByWordWrapping];
    
    UILabel *upperLabel = [[UILabel alloc] init];
    
    upperLabel.frame = CGRectMake(0,topView.frame.size.height+ 20, screenWidth,expectedLabelSize.height);
    upperLabel.opaque = NO;
    upperLabel.font = [UIFont systemFontOfSize:20];
    upperLabel.numberOfLines = 0;
    upperLabel.textAlignment = UITextAlignmentCenter;
    upperLabel.textColor = [UIColor whiteColor];
    upperLabel.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.0];
    [upperLabel setText:@"Keep the product upright and in good Lighting for better scanning"];
    [backgroungBlurrView addSubview:upperLabel];
    ...
    bottomLabel.frame = CGRectMake(0,topView.frame.size.height+r.origin.y+r.size.height-210, screenWidth,expectedLabelSize.height);
    bottomLabel.opaque = NO;
    bottomLabel.font = [UIFont systemFontOfSize:20];
    bottomLabel.numberOfLines = 0;
    bottomLabel.textAlignment = UITextAlignmentCenter;
    bottomLabel.textColor = [UIColor whiteColor];
    bottomLabel.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.0];
    [bottomLabel setText:@"Keep the product upright and in good Lighting for better scanning"];
    [backgroungBlurrView addSubview:bottomLabel];
    
    
    
    
    
    UILabel *ScanBarCodeLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2-self.view.frame.size.width/4, 0, 200, 45)];
    ScanBarCodeLabel.font = [UIFont systemFontOfSize:25.0f];
    ScanBarCodeLabel.textColor = [UIColor whiteColor];
    
    //    ScanBarCodeLabel.backgroundColor = [UIColor redColor];
    ScanBarCodeLabel.text = @"Scan BarCode";
    [topView addSubview: ScanBarCodeLabel];
       ...
       self.parentViewController.modalViewController == self)
    {
        ...
        r.origin.y+=screenHeight/4;
        
        r.size.height -= screenHeight/2;
    }
